...
# pr() argument names
# Locate pr()'s signature by CONTENT, not by hardcoded line numbers: clus.jsonnet
# is edited often and fixed line numbers silently start reading the wrong
# function.  pr() runs from its "pr(anodes, ..." declaration to the
# "clus_pr(anodes," call that ends it.
...
def toolkit_commit(path):
    """Read the toolkit HEAD without needing a git binary -- `git` is not on PATH
    in the bare SL7 container, and a provenance header that degrades to '?' loses
    the one fact that says WHICH version of the operating point this is."""
        ...
        if os.path.isdir(g):
            try:
                ...
                if head.startswith('ref: '):
                    ref = head[5:]
                    rp = os.path.join(g, ref)
                    if os.path.exists(rp):
                        return open(rp).read().strip()[:9]
                    for ln in open(os.path.join(g, 'packed-refs')):
...
src = toolkit_commit(clus_path)
if not src:
    sys.exit("FATAL: cannot determine the toolkit commit for the provenance header.")
...
if unmapped or ambiguous:
    print("  unmapped  : %s" % unmapped[:8]); print("  ambiguous : %s" % ambiguous[:8])
    sys.exit("FATAL: refusing to write a partial operating point.")

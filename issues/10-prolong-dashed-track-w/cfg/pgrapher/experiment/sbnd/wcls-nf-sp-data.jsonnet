...
if roi == "dnn" then {
    ...
    sparse: true,
    // issue #10: the magnify dump needs the ROI-debug stages.  Unlike the "dnn"
    // and "both" branches above we must NOT blank the *_tag fields -- an empty
    // tag silently drops that stage from the dump.
    use_roi_debug_mode: true,
};
...
local magoutput = 'magnify.root';  // issue #10 campaign: per-event cwd
...
[
  g.pipeline([
               ...
               sinks.debug_pipe[n],   // needs use_roi_debug_mode=true, set above
             ],
             'nfsp_pipe_%d' % n)
  for n in std.range(0, std.length(tools.anodes) - 1)
];

...
### 1.5 Input: the official Gen2 reco1 (measured) -- SUPERSEDED for the production

> **Decision 2026-09-17 (Haiwang): the 1M-event production runs on new Gen2 CV samples
> generated by Avinay, not on twester's `sbnd_gen2_prod1-3`.** Template = Avinay's
> 50-event bench `abhat/sbnd/gen2-cv-50evt-benchmark-20260915/reco1/reco1-gen2cv-50evt-bench-20260915.root`
> (1.42 GB, run 1 subrun 1, 50 events with sparse ids 1-165, same reco1 products;
> the 1-step chain on Aurora is exact 50/50 vs FNAL on it, 20/50 candidates). The
> survey below of twester's set stays as the reference for the file layout and the
> per-event sizes until Avinay's production exists; redo it then (file count,
> events/file, total size, directory layout) and revise decision 1.
...
1. ~~Sample for the 1M events: prod1 `reco1/000000-000075`~~ **Decided 2026-09-17: Avinay's
   new Gen2 CV production.** Open: its location on Flare, file count, events/file,
   and when the first chunk lands (A4 needs ~1,000 files).
...
| A3 MC validations vs FNAL (issue 26 plan) | **MC CV pilot exact 18/18; Avinay's 50-event Gen2 CV bench exact 50/50** (2026-09-17; `deep_compare`, Bee, checksums, census all 0 differences; Bee https://www.phy.bnl.gov/twister/bee/set/1e0bf31b-6588-4534-92c0-9483ca33fd16/event/list/) |
